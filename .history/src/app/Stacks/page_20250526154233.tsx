              ...
              <div className="grid grid-cols-2 gap-4">
                {category.technologies.map((tech, idx) => (
                  ...
                  >
                    {tech.imageUrl ? (
       <Image
  src={tech.imageUrl || "/logos/default.png"} // fallback local logo
  alt={tech.name}
  width={50}
  height={50}
...
/>
    ...
    <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <h2 className="text-3xl md:text-4xl font-bold mb-4">Why This Stack?</h2>
            <p className="text-lg text-gray-600 max-w-3xl mx-auto">
              Our technology choices are driven by a commitment to performance, scalability, security, and developer experience, ensuring we deliver robust and innovative solutions.
            </p>
          </motion.div>
          <motion.div 
          ...
          >
            {[
              { icon: <Zap className="h-8 w-8 text-yellow-500" />, title: "Performance", description: "Leveraging modern tools for fast load times and smooth interactions." },
              { icon: <Layers className="h-8 w-8 text-blue-500" />, title: "Scalability", description: "Building on flexible architectures that grow with your needs." },
              { icon: <Shield className="h-8 w-8 text-green-500" />, title: "Security", description: "Prioritizing data protection and robust security measures." }
            ].map((item, index) => (
              <motion.div 
                key={index}

"use client";

import React from "react";
import { motion } from "framer-motion";
import Image from "next/image";

const containerVariants = {
  hidden: {},
  visible: {
    transition: {
      staggerChildren: 0.5,
      delayChildren: 0.3,
    },
  },
};

const fadeInUp = {
  hidden: { opacity: 0, y: 30 },
  visible: {
    opacity: 1,
    y: 0,
    transition: { duration: 0.6, ease: "easeOut" },
  },
};

const zoomIn = {
  hidden: { opacity: 0, scale: 0.9 },
  visible: {
    opacity: 1,
    scale: 1,
    transition: { duration: 0.6, ease: "easeOut" },
  },
};

const OurStory: React.FC = () => {
  return (
    <section className="md:py-20 py-10">
      <motion.div
        className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 space-y-20"
        variants={containerVariants}
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true, amount: 0.2 }}
      >
        <motion.div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-stretch text-center md:text-left">
          {/* Text Box */}
          <motion.div
            variants={fadeInUp}
            className="flex flex-col justify-center h-full"
          >
            <div className="h-full flex flex-col justify-center">
              <h2 className="text-3xl md:text-4xl font-bold mb-6 text-primary dark:text-white">
                IT Specialized Company Based in Laos
              </h2>
              <div className="mb-6 text-xl md:text-2xl font-bold text-gray-800 dark:text-white">
                <p>
                  We are a Laos-based IT startup aiming for the global market.
                </p>
              </div>
              <p>
                We are targeting not only the local market in Laos but also
                neighboring Southeast Asian countries and the Northeast Asian
                market, including South Korea.
              </p>
            </div>
          </motion.div>

          {/* Image Box */}
          <motion.div
            variants={zoomIn}
            className="relative h-[300px] sm:h-[300px] md:h-[400px] w-full"
          >
            <div className="relative w-full h-full rounded-2xl border overflow-hidden shadow-xl">
              <Image
                src="/images/Start.gif"
                alt="Animated team launch illustration"
                fill
                className="object-contain dark:hidden"
                priority
              />
              <Image
                src="/images/Start.gif"
                alt="Animated team launch illustration"
                fill
                className="object-contain hidden dark:block"
              />
            </div>
          </motion.div>
        </motion.div>

        {/* Block 2 */}
        <motion.div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-stretch text-center md:text-left">
          {/* Image Box */}
          <motion.div
            variants={zoomIn}
            className="relative order-last lg:order-first h-[300px] sm:h-[300px] md:h-[400px] w-full"
          >
            <div className="relative w-full h-full rounded-2xl border overflow-hidden shadow-xl">
              <Image
                src="/images/Animated IT.gif"
                alt="Team coding interface animation"
                fill
                className="object-contain dark:hidden"
              />
              <Image
                src="/images/Animated IT.gif"
                alt="Team coding interface animation"
                fill
                className="object-contain hidden dark:block"
              />
            </div>
          </motion.div>

          {/* Text Box */}
          <motion.div
            variants={fadeInUp}
            className="flex flex-col justify-center h-full"
          >
            <div className="h-full flex flex-col justify-center">
              <h2 className="text-3xl md:text-4xl font-bold mb-6 text-primary dark:text-white">
                Anything related to IT that our customers need and desire.
              </h2>
              <div className="mb-6 text-xl md:text-2xl font-bold text-gray-800 dark:text-white">
                <p>We can provide anything related to IT for our customers.</p>
              </div>
              <p>
                We provide comprehensive outsourcing development services in the
                IT sector as well as consulting on IT business.
              </p>
            </div>
          </motion.div>
        </motion.div>

        {/* Block 3 */}
        <motion.div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-stretch text-center md:text-left">
          {/* Text Box */}
          <motion.div
            variants={fadeInUp}
            className="flex flex-col justify-center h-full"
          >
            <div className="h-full flex flex-col justify-center">
              <h2 className="text-3xl md:text-4xl font-bold mb-6 text-primary dark:text-white">
                Bringing various IT services to Lao society
              </h2>
              <div className="mb-6 text-xl md:text-2xl font-bold text-gray-800 dark:text-white">
                <p>We are also developing and operating our own IT services.</p>
              </div>
              <p>
                We develop and implement IT services utilizing various
                technologies and operate them within Lao society.
              </p>
            </div>
          </motion.div>

          {/* Image Box */}
          <motion.div
            variants={zoomIn}
            className="relative h-[300px] sm:h-[300px] md:h-[400px] w-full"
          >
            <div className="relative w-full h-full rounded-2xl border overflow-hidden shadow-xl">
              <Image
                src="/images/Animated IT Service.gif"
                alt="IT service platform animation"
                fill
                className="object-contain dark:hidden"
              />
              <Image
                src="/images/Animated IT Service.gif"
                alt="IT service platform animation"
                fill
                className="object-contain hidden dark:block"
              />
            </div>
          </motion.div>
        </motion.div>
      </motion.div>
    </section>
  );
};

export default OurStory;

    ...
    <nav className="sticky top-0 z-50 w-full bg-white/50 dark:bg-black/50 dark:border-strokedark  backdrop-blur-md border-b border-border">
      ...
      {isOpen && (
        <motion.div
          ...
          className="md:hidden"
        >
          <div className="px-4 pt-4 pb-6 space-y-3 sm:px-6 bg-white dark:bg-blacksection shadow-xl rounded-lg border border-gray-200 dark:border-strokedark transition-all duration-300">
            ...
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`block px-4 py-2.5 rounded-md text-base font-medium tracking-wide transition-colors duration-200 ${
                  pathname === link.href
                    ? "bg-primary text-primary-foreground shadow-sm"
                    : "text-gray-700 dark:text-gray-300 hover:bg-muted hover:text-muted-foreground"
                }`}

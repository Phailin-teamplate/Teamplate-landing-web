...
useEffect(() => {
  ...
  // Cleanup ถ้า component นี้ถูก unmount
  return () => {
    document.body.style.overflow = "";
  };
}, [isOpen]);
...
>
  <div
    className="w-full h-full px-6 py-8 flex flex-col bg-white dark:bg-blacksection text-gray-800 dark:text-gray-100"
    onClick={(e) => e.stopPropagation()}
  >
    {/* Header & nav as before */}
  </div>
</motion.div>
    ...
    >
      <div className="min-h-screen w-full px-6 py-8 flex flex-col bg-white dark:bg-blacksection text-gray-800 dark:text-gray-100 transition-colors duration-300">
        {/* Header */}
            ...
            <Link
              ...
              className={`w-full text-center px-5 py-3 rounded-lg text-lg font-medium tracking-wide transition-all duration-200 shadow-sm ${
                pathname === link.href
                  ...
                  : "text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-muted hover:text-black dark:hover:text-white"
              }`}
            >
              {link.label}
            </Link>
